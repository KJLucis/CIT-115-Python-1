...
#The previous print outputs are converted to string with formatting and
...
def writeOutput (sName,iGallons,fLaborHours,fPaintCost,fLaborCost,fSalesTax,fTotalCost):
    Job_File = open(sName+"_PaintJobOutput.txt", "w")
    Job_File.write("Gallons of paint:" + str(iGallons) + '\n')
    Job_File.write("Hours of Labor: "+str(fLaborHours)+'\n')
    Job_File.write("Paint charges: $"+str(format(fPaintCost,',.2f'))+'\n')
    Job_File.write("Labor charges: $"+str(format(fLaborCost,',.2f'))+'\n')
    Job_File.write("Tax: $"+str(format(fSalesTax,',.2f'))+'\n')
    Job_File.write("Total Cost: $"+str(format(fTotalCost,',.2f'))+'\n')
    Job_File.close()
    print("File: "+sName+"_PaintJobOutput.txt was created.")
